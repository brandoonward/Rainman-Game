...
`ifdef SYNTHESIS
    ...
    parameter imem_init="imem.mem",  // version of program for synthesis, with pauses after each keystroke
    //parameter imem_init="imem_fullIO_test.mem",  // version of program for synthesis, with pauses after each keystroke
`else
...
`endif
    ...
    parameter smem_init="smem_final.mem", 	// text file to initialize screen memory
    parameter bmem_init="bmem_final.mem" 	// text file to initialize bitmap memory
)(    
   ...
   wire enable = 1'b 1;
   wire [wordsize-1:0] pc, instr, mem_readdata, mem_writedata, mem_addr, keyb_char, accel_val, period, display_val;   
...
`endif
   ...
   memIO #(.wordsize(wordsize), .dmem_size(dmem_size), .dmem_init(dmem_init), .Nchars(Nchars), .smem_size(smem_size), .smem_init(smem_init)
       ...
       .vga_addr(smem_addr), .vga_readdata(charcode), .keyb_char, .accel_val, .period, .display_val(display_val), .LED(LED));
   ...
   // Sound
   sound sound(.clock100(clk100), .period, .*);
   ...
   display8digit disp(.val(display_val), .clk(clk100), .segments, .digitselect);
